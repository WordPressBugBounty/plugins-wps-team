...
abstract class Controls_Stack extends Base_Object {
	...
	/**
	 * Responsive 'large-mobile' device name.
	 */
	const RESPONSIVE_SMALL_TABLET = 'small_tablet';
	 ...
	 * The configuration.
	 *
	 * Holds the configuration used to generate the editor. It includes
	 ...
	 * Get the type.
	 *
	 * Retrieve the type, e.g. 'stack', 'section', 'shortcode' etc.
	 *
	 * @since 1.4.0
	 * @access public
	 * @static
	...
	public function add_control( $id, array $args, $options = [] ) {
		...
		if ( empty( $args['type'] ) || ! in_array( $args['type'], [ Controls_Manager::SECTION ], true ) ) {
	...
	final public function add_responsive_control( $id, array $args, $options = [] ) {
		...
		foreach ( $devices as $device_name ) {
			...
			if ( isset( $control_args[ $device_name . '_unit' ] ) ) {
				$control_args['unit'] = $control_args[ $device_name . '_unit' ];
			}

			unset( $control_args['desktop_default'] );
			unset( $control_args['tablet_default'] );
			unset( $control_args['small_tablet_default'] );
			unset( $control_args['mobile_default'] );

			unset( $control_args['desktop_unit'] );
			unset( $control_args['tablet_unit'] );
			unset( $control_args['small_tablet_unit'] );
			unset( $control_args['mobile_unit'] );
	...
	public function values_to_settings( $values ) {

		$settings = $this->get_controls();
		...
		foreach ( $settings as $control ) {
			...
			if ( in_array( $control['name'], array_keys($values) ) ) {
				
				$setting = $values[ $control['name'] ];

				if ( isset($setting['value']) ) {
					$settings[ $control['name'] ]['value'] = $setting['value'];
				}

				if ( isset($setting['unit']) ) {
					$settings[ $control['name'] ]['unit'] = $setting['unit'];
				}
	...
	public function get_settings_value() {
		...
		foreach ( $this->get_controls() as $control ) {
			...
			if ( is_array($control['name']) && isset( $control['name']['type'] ) ) continue;

			$setting = $settings[ $control['name'] ];
			
			if ( gettype($setting['value']) == 'array' && array_key_exists('value', $setting['value']) ) {
				$_settings[ $control['name'] ] = $setting['value'];
			} else {
				$_settings[ $control['name'] ] = [ 'value' => $setting['value'] ];
			}
			
			if ( ! empty($setting['unit']) ) {
				$_settings[ $control['name'] ][ 'unit' ] = $setting['unit'];
			}
	...
	public function get_stack_formed_values( $settings ) {
	...
	public function get_display_formated_values( $settings = [] ) {
		if ( empty($settings) ) $settings = $this->get_settings_value();
		foreach ( $settings as $key => $val ) {
	...
	public function start_controls_section( $section_id, array $args = [] ) {
		 ...
		 */
		do_action( 'wpspeedo_team/element/before_section_start', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action( "wpspeedo_team/element/{$section_name}/{$section_id}/before_section_start", $this, $args );
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action( 'wpspeedo_team/element/after_section_start', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action( "wpspeedo_team/element/{$section_name}/{$section_id}/after_section_start", $this, $args );
	}
	...
	public function end_controls_section() {
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action( 'wpspeedo_team/element/before_section_end', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action( "wpspeedo_team/element/{$stack_name}/{$section_id}/before_section_end", $this, $args );
		 ...
		 * @param array          $args       Section arguments.
		 */
		do_action( 'wpspeedo_team/element/after_section_end', $this, $section_id, $args );
		 ...
		 * @param array          $args Section arguments.
		 */
		do_action( "wpspeedo_team/element/{$stack_name}/{$section_id}/after_section_end", $this, $args );
	}
	 ...
	 * developers use this method to register controls in a shortcode.
	 *
	...
	protected function get_init_settings() {
		...
		foreach ( $this->get_controls() as $control ) {
			...
			$value = $control_obj->get_value( $control, $settings );
	...
	protected function handle_control_position( array $args, $control_id, $overwrite ) {
		...
		if ( isset( $args['type'] ) && in_array( $args['type'], [ Controls_Manager::SECTION ], true ) ) {
			return $args;
		}
	...
	protected function init( $data ) {
		...
		$this->id = $data['id'];
		$this->data['settings'] = $this->get_init_settings();
	}
