...
class API {
    ...
    public function sanitize_settings( $settings ) {
        ...
        if ( !empty($settings['post_type_slug']) ) {
            if ( $settings['post_type_slug'] === '/' ) $settings['post_type_slug'] = '';
        ...
        $settings_editor = new Settings_Editor([ 'id' => 'fake', 'settings' => $settings ]); // This class will handle the Sanitization & Validation.
        ...
        if ( isset( $settings['breakpoint_mobile_max'], $settings['breakpoint_small_tablet_max'], $settings['breakpoint_tablet_max'] ) ) {
            $normalized = Utils::normalize_breakpoint_widths(
                (int) $settings['breakpoint_mobile_max'],
                (int) $settings['breakpoint_small_tablet_max'],
                (int) $settings['breakpoint_tablet_max']
            );
    ...
    public function save_settings( $settings ) {
        $settings = $this->sanitize_settings( $settings ); // Sanitization & Validation done manually.
    ...
    public function ajax_get_shortcodes() {
        ...
        foreach( $shortcodes as &$shortcode ) {
            ...
            $shortcode['settings'] = $this->validate_shortcode( $shortcode )->get_settings_value(); // Settings will be Sanitized & Validated by Shortcode_Editor class.
        }

        if ( ! wp_doing_ajax() ) return $shortcodes;
    ...
    public function fetch_shortcode( $shortcode_id ) {
        ...
        if ( false === $shortcode ) {
            $shortcode = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}wps_team WHERE id = %d LIMIT 1", $shortcode_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
            if ( ! empty($shortcode) ) {
    ...
    public function ajax_update_shortcode() {
        ...
        if ( ! empty( $settings ) ) {
            $shortcode['settings'] = $settings; // Will be sanitized by Shortcode_Editor class
        ...
        wp_send_json_success(
            [
                'message' => sprintf(
                    '<strong>%s</strong> %s',
                    esc_html_x( 'Congrats!', 'Dashboard', 'wps-team' ),
                    esc_html_x( 'Shortcode updated successfully', 'Dashboard', 'wps-team' )
                ),
                'data'    => $return_data,
            ]
        );
    ...
    public function validate_shortcode( $shortcode ) {
        ...
        $shortcode = new Shortcode_Editor( $shortcode ); // This class will handle the Sanitization & Validation.
    ...
    public function ajax_create_shortcode() {
        ...
        $shortcode = $this->validate_shortcode( [
            'id'       => uniqid(), // Fake ID
            'name'     => $shortcode_name,
            'settings' => $raw_settings // Will be sanitized & validated in Shortcode_Editor class
        ]);
        ...
        $wpdb->insert( "{$wpdb->prefix}wps_team", $data, $this->db_columns_format() ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
        ...
        wp_send_json_success( [
            'message' => sprintf(
                '<strong>%s</strong> %s',
                _x( 'Congrats!', 'Dashboard', 'wps-team' ),
                _x( 'Shortcode created successfully', 'Dashboard', 'wps-team' )
            ),
    ...
    public function ajax_delete_shortcode() {
        ...
        wp_send_json_success([
            'message' => sprintf(
                '<strong>%s</strong> %s',
                esc_html_x('Done!', 'Dashboard', 'wps-team'),
                esc_html_x('Shortcode deleted successfully', 'Dashboard', 'wps-team')
            ),
            'data' => [ 'id' => $id ]
        ]);
    }
    ...
    public function ajax_clone_shortcode() {
        ...
        if ( ! $clone_id ) wp_send_json_error( _x('Clone Id not provided', 'Dashboard', 'wps-team'), 400 );

        $clone_shortcode = $this->fetch_shortcode( $clone_id );

        if ( empty($clone_shortcode) ) wp_send_json_error( _x('Clone shortcode not found', 'Dashboard', 'wps-team'), 404 );

        $shortcode = $this->validate_shortcode([
            'id' => uniqid(), // Fake ID
            'name' => $clone_shortcode['name'] .' '. _x('- Cloned', 'Editor', 'wps-team'),
            'settings' => $clone_shortcode['settings'] // Settings will be Sanitized & Validated by Shortcode_Editor class.
        ]);
    ...
    public function ajax_temp_save_settings() {
        ...
        if ( empty($temp_key) ) wp_send_json_error( _x('No temp key provide', 'Editor', 'wps-team'), 400 );
        if ( empty($settings) ) wp_send_json_error( _x('No temp settings provided', 'Editor', 'wps-team'), 400 );

        $shortcode = $this->validate_shortcode([
            'id' => $temp_key, // Fake ID
            'name' => 'Fake Name',
            'settings' => $settings // Settings will be Sanitized & Validated by Shortcode_Editor class.
        ]);
