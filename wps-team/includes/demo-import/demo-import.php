...
class Demo_Import {
    ...
    public function maybe_disable_notice() {
        ...
        if ( $demo_import_notice === false || ! $demo_import_notice->is_active ) return; // Already Disabled the Notice

        $installed_age = Utils::get_timestamp_diff( Utils::get_installed_time() );
        if ( $installed_age > 7 ) return $this->disable_notice();

        if ( Utils::get_demo_data_status( 'post_data' ) || Utils::get_demo_data_status( 'shortcode_data' ) ) return $this->disable_notice();

        $post_count = Utils::get_posts( [ 'posts_per_page' => 1 ] )->post_count;
        wp_reset_postdata();
    ...
    public function remove_dummy_indicator( $post_id ) {
        ...
        if ( empty( get_post_meta($post_id, self::$key . '--dummy', true) ) ) return;
        
        $taxonomies = $this->get_taxonomy_list();

        // Remove dummy indicator from texonomies
        $dummy_terms = wp_get_post_terms( $post_id, $taxonomies, [ 'fields' => 'ids', 'meta_key' => self::$key . '--dummy', 'meta_value' => 1 ]); // phpcs:ignore
        ...
        if ( !empty($thumbnail_id) ) delete_post_meta( $thumbnail_id, self::$key . '--dummy', 1 );
    ...
    public function ajax_import_demo_data() {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            ...
            if ( wp_doing_ajax() ) wp_send_json_error( $message, 403 );
        ...
        if ( get_option(self::$key . '_dummy_post_data_created') !== false || get_transient(self::$key . '_dummy_post_data_creating') !== false ) {
            ...
            $message_202 = _x( 'Dummy team members already imported', 'Settings: Tools', 'wps-team' );

            if ( wp_doing_ajax() ) wp_send_json_success( $message_202, 202 );

            return [ 'status' => 202, 'message' => $message_202 ];
        }
        
        $this->create_dummy_attachments();

        $message = _x( 'Dummy team members imported', 'Settings: Tools', 'wps-team' );

        if ( wp_doing_ajax() ) wp_send_json_success( $message, 200 );
    ...
    public function ajax_remove_demo_data() {
        ...
        $message = _x( 'Dummy team members deleted', 'Settings: Tools', 'wps-team' );
    ...
    public function get_attachment_id_by_filename( $filename ) {
        ...
        if ( empty($attachments = $this->get_dummy_attachments()) ) return '';
    ...
    public function get_dummy_posts() {
        ...
        $posts = get_posts( array( 'numberposts' => -1, 'post_type'   => Utils::post_type_name(), 'meta_key' => self::$key . '--dummy', 'meta_value' => 1, ) ); // phpcs:ignore
    ...
    public function create_dummy_attachments() {
        ...
        $attachment_files = ['wpspeedo-team-01.jpg', 'wpspeedo-team-02.jpg', 'wpspeedo-team-03.jpg', 'wpspeedo-team-04.jpg', 'wpspeedo-team-05.jpg', 'wpspeedo-team-06.jpg', 'wpspeedo-team-07.jpg', 'wpspeedo-team-08.jpg', 'wpspeedo-team-09.jpg', 'wpspeedo-team-10.jpg', 'wpspeedo-team-11.jpg', 'wpspeedo-team-12.jpg'];
    ...
    public function get_dummy_attachments() {
        ...
        $attachments = get_posts( array( 'numberposts' => -1, 'post_type'   => 'attachment', 'post_status' => 'inherit', 'meta_key' => self::$key . '--dummy', 'meta_value' => 1 ) ); // phpcs:ignore
    ...
    public function create_dummy_terms() {
        ...
        $terms = [
            [ 'name' => 'Accountant', 'slug' => 'accountant', 'group' => 'wps-team-group' ],
            [ 'name' => 'Business', 'slug' => 'business', 'group' => 'wps-team-group' ],
            [ 'name' => 'Manager', 'slug' => 'manager', 'group' => 'wps-team-group' ],
            [ 'name' => 'Marketer', 'slug' => 'marketer', 'group' => 'wps-team-group' ]
        ];
    ...
    public function get_dummy_terms() {
        ...
        $terms = get_terms( array( 'taxonomy' => $taxonomies, 'hide_empty' => false, 'meta_key' => self::$key . '--dummy', 'meta_value' => 1 ) ); // phpcs:ignore
