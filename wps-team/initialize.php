...
class Initialize {
    ...
    public function load_textdomain() {
        ...
        load_plugin_textdomain( 'wps-team', false, dirname( plugin_basename( WPS_TEAM_FILE ) ) . '/languages/' );
    }
    ...
    public function fail_php_version() {
        $message = '<strong>WPS Team</strong> ' . sprintf(
            ...
            esc_html_x(
                'plugin requires PHP version %s+, plugin is currently NOT RUNNING.',
                'Dashboard',
                'wps-team'
            ),
            '7.4'
    ...
    public function fail_wp_version() {
        $message = '<strong>WPS Team</strong> ' . sprintf(
            ...
            esc_html_x(
                'plugin requires WordPress version %s+. Because you are using an earlier version, the plugin is currently NOT RUNNING.',
                'Dashboard',
                'wps-team'
            ),
            '5.9'
    ...
    function maybe_update_pro_status() {
        ...
        if ( $is_pro_active === $current_pro_status ) return;
